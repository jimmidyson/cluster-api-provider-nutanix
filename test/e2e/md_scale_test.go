...
var _ = Describe(
	...
	Label("md-scale-out-in", "slow", "network"),
	func() {
...
var _ = Describe(
	...
	Label("md-scale-up-down", "slow", "network", "only-for-validation"),
	func() {
		...
		BeforeEach(func() {
			...
			Expect(
				input.E2EConfig,
			).ToNot(BeNil(), "Invalid argument. input.E2EConfig can't be nil when calling %s spec", specName)
			Expect(
				input.ClusterctlConfigPath,
			).To(BeAnExistingFile(), "Invalid argument. input.ClusterctlConfigPath must be an existing file when calling %s spec", specName)
			Expect(
				input.BootstrapClusterProxy,
			).ToNot(BeNil(), "Invalid argument. input.BootstrapClusterProxy can't be nil when calling %s spec", specName)
			Expect(
				os.MkdirAll(input.ArtifactFolder, 0o750),
			).To(Succeed(), "Invalid argument. input.ArtifactFolder can't be created for %s spec", specName)
			Expect(input.E2EConfig.Variables).To(HaveKey(KubernetesVersion))
			Expect(
				input.E2EConfig.Variables,
			).To(HaveValidVersion(input.E2EConfig.GetVariable(KubernetesVersion)))
		...
		It(
			...
			func() {
				...
				clusterctl.ApplyClusterTemplateAndWait(
					ctx,
					clusterctl.ApplyClusterTemplateAndWaitInput{
						...
						ConfigCluster: clusterctl.ConfigClusterInput{
							...
							KubernetesVersion: input.E2EConfig.GetVariable(
								KubernetesVersion,
							),
						...
						WaitForClusterIntervals: input.E2EConfig.GetIntervals(
							specName,
							"wait-cluster",
						),
				...
				Expect(
					clusterResources.MachineDeployments[0].Spec.Replicas,
				).To(Equal(pointer.Int32Ptr(10)))

				By("Scaling the MachineDeployment out to 20")
				framework.ScaleAndWaitMachineDeployment(
					ctx,
					framework.ScaleAndWaitMachineDeploymentInput{
						ClusterProxy:      input.BootstrapClusterProxy,
