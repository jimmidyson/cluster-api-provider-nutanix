...
var _ = Describe(
	"Nutanix regression tests",
	Label("capx-regression-test", "regression", "slow", "network"),
	func() {
		...
		AfterEach(func() {
			...
			Expect(
				bcpClient.Delete(ctx, myCluster),
			).To(SatisfyAny(Succeed(), MatchError(ContainSubstring("not found"))))
		...
		It(
			"[Test regression: credentials-delete-race-regression] Create a cluster and delete the credentials secret before delete the cluster",
			func() {
