...
func GetVMUUID(nutanixMachine *infrav1.NutanixMachine) (string, error) {
	...
	if vmUUID != "" {
		if _, err := uuid.Parse(vmUUID); err != nil {
			return "", fmt.Errorf(
				"VMUUID was set but was not a valid UUID: %s err: %v",
				vmUUID,
				err,
			)
		}
	...
	if vmUUID != "" {
		...
		if vm == nil {
			return nil, fmt.Errorf(
				"no vm %s found with UUID %s but was expected to be present",
				vmName,
				vmUUID,
			)
		}
		...
		if *vm.Spec.Name != vmName && *vm.Spec.Name != nutanixMachine.Name {
			return nil, fmt.Errorf(
				"found VM with UUID %s but name %s did not match %s",
				vmUUID,
				*vm.Spec.Name,
				vmName,
			)
...
func FindVMByName(
	...
	if len(res.Entities) > 1 {
		return nil, fmt.Errorf(
			"error: found more than one (%v) vms with name %s",
			len(res.Entities),
			vmName,
		)
	}
...
func GetPEUUID(
	...
	if peUUID != nil && *peUUID != "" {
		...
		if err != nil {
			if strings.Contains(fmt.Sprint(err), "ENTITY_NOT_FOUND") {
				return "", fmt.Errorf(
					"failed to find Prism Element cluster with UUID %s: %v",
					*peUUID,
					err,
				)
			}
	...
	return "", fmt.Errorf(
		"failed to retrieve Prism Element cluster by name or uuid. Verify input parameters",
	)
}
...
func CreateSystemDiskSpec(imageUUID string, systemDiskSize int64) (*nutanixClientV3.VMDisk, error) {
	...
	if systemDiskSize <= 0 {
		return nil, fmt.Errorf(
			"invalid system disk size: %d. Provide in XXGi (for example 70Gi) format instead",
			systemDiskSize,
		)
	}
...
func GetSubnetUUID(
	...
	client *nutanixClientV3.Client,
	peUUID string,
	subnetName, subnetUUID *string,
) (string, error) {
	var foundSubnetUUID string
	if subnetUUID == nil && subnetName == nil {
		return "", fmt.Errorf(
			"subnet name or subnet uuid must be passed in order to retrieve the subnet",
		)
	}
	if subnetUUID != nil {
...
func GetImageUUID(
	ctx context.Context,
	client *nutanixClientV3.Client,
	imageName, imageUUID *string,
) (string, error) {
	var foundImageUUID string

	if imageUUID == nil && imageName == nil {
		return "", fmt.Errorf(
			"image name or image uuid must be passed in order to retrieve the image",
		)
	}
	if imageUUID != nil {
...
func HasTaskInProgress(
	...
	if taskStatus != taskSucceededMessage {
		log.V(1).
			Info(fmt.Sprintf("VM task with UUID %s still in progress: %s", taskUUID, taskStatus))
		return true, nil
	}
	return false, nil
}
...
func GetTaskUUIDFromVM(vm *nutanixClientV3.VMIntentResponse) (string, error) {
	...
	default:
		return "", fmt.Errorf(
			"invalid type found for task uuid extracted from vm %s: %v",
			vmName,
			t,
		)
	}
...
func GetObsoleteDefaultCAPICategoryIdentifiers(
	clusterName string,
...
) ([]*nutanixClientV3.CategoryValueStatus, error) {
...
func getCategoryKey(
	...
	client *nutanixClientV3.Client,
	key string,
) (*nutanixClientV3.CategoryKeyStatus, error) {
...
func getCategoryValue(
	...
	if err != nil {
		if !strings.Contains(fmt.Sprint(err), "CATEGORY_NAME_VALUE_MISMATCH") {
			return nil, fmt.Errorf(
				"failed to retrieve category value %s in category %s. error: %v",
				value,
				key,
				err,
			)
...
func deleteCategoryKeyValues(
	ctx context.Context,
	client *nutanixClientV3.Client,
	categoryIdentifiers []*infrav1.NutanixCategoryIdentifier,
	ignoreKeyDeletion bool,
) error {
	log := ctrl.LoggerFrom(ctx)
	...
	for key, values := range groupCategoriesByKey {
		...
		for _, value := range values {
			...
			if categoryValue == nil {
				log.V(1).
					Info(fmt.Sprintf("Category with value %s in category %s not found. Already deleted?", value, key))
				continue
			}
		...
		if !ignoreKeyDeletion {
			...
			categoryKeyValues, err := client.V3.ListCategoryValues(
				ctx,
				key,
				&nutanixClientV3.CategoryListMetadata{},
			)
			if err != nil {
			...
			if len(categoryKeyValues.Entities) > 0 {
				errorMsg := fmt.Errorf(
					"cannot remove category with key %s because it still has category values assigned",
					key,
				)
			...
			log.V(1).
				Info(fmt.Sprintf("No values assigned to category. Removing category with key %s", key))
...
func DeleteCategories(
	ctx context.Context,
	client *nutanixClientV3.Client,
	categoryIdentifiers, obsoleteCategoryIdentifiers []*infrav1.NutanixCategoryIdentifier,
) error {
...
func getOrCreateCategory(
	...
	if categoryIdentifier == nil {
		return nil, fmt.Errorf(
			"category identifier cannot be nil when getting or creating categories",
		)
	}
	if categoryIdentifier.Key == "" {
		return nil, fmt.Errorf(
			"category identifier key must be set when when getting or creating categories",
		)
	}
	...
	if err != nil {
		errorMsg := fmt.Errorf(
			"failed to retrieve category with key %s. error: %v",
	...
	if categoryKey == nil {
		...
		if err != nil {
			errorMsg := fmt.Errorf(
				"failed to create category with key %s. error: %v",
				categoryIdentifier.Key,
				err,
			)
	...
	if categoryValue == nil {
		categoryValue, err = client.V3.CreateOrUpdateCategoryValue(
			ctx,
			*categoryKey.Name,
			&nutanixClientV3.CategoryValue{
				Description: utils.StringPtr(infrav1.DefaultCAPICategoryDescription),
		...
		if err != nil {
			errorMsg := fmt.Errorf(
				"failed to create category value %s in category key %s: %v",
				categoryIdentifier.Value,
				categoryIdentifier.Key,
				err,
			)
...
func GetCategoryVMSpec(
	...
	for _, ci := range categoryIdentifiers {
		...
		if err != nil {
			errorMsg := fmt.Errorf(
				"error occurred while to retrieving category value %s in category %s. error: %v",
				ci.Value,
				ci.Key,
				err,
			)
		...
		if categoryValue == nil {
			errorMsg := fmt.Errorf(
				"category value %s not found in category %s. error",
				ci.Value,
				ci.Key,
			)
...
func GetProjectUUID(
	ctx context.Context,
	client *nutanixClientV3.Client,
	projectName, projectUUID *string,
) (string, error) {
...
func hasPEClusterServiceEnabled(
	peCluster *nutanixClientV3.ClusterIntentResponse,
	serviceName string,
) bool {
...
func GetGPU(
	...
	if gpuDeviceID == nil && gpuDeviceName == nil {
		return nil, fmt.Errorf(
			"gpu name or gpu device ID must be passed in order to retrieve the GPU",
		)
	}
	...
	if len(allGPUs) == 0 {
		return nil, fmt.Errorf(
			"no available GPUs found in Prism Element cluster with UUID %s",
			peUUID,
		)
	}
	for _, peGPU := range allGPUs {
		...
		if (gpuDeviceID != nil && *peGPU.DeviceID == *gpuDeviceID) ||
			(gpuDeviceName != nil && *gpuDeviceName == peGPU.Name) {
	...
	return nil, fmt.Errorf(
		"no available GPU found in Prism Element that matches required GPU inputs",
	)
}
